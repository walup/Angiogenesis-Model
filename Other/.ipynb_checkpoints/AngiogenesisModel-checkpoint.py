...
class AngiogenesisModel:
    ...
    def getPictureTipStalkCells(self):
        tipCellColor = np.array([219/255, 50/255, 252/255])
        vasculatureColor = np.array([201/255, 74/255, 0/255])
        backgroundColor = [1, 1, 1]
        minConcentrationColor = np.array([255/255, 255/255, 255/255])
        maxConcentrationColor = np.array([255/255, 196/255, 0/255])
        
        picture = np.ones((self.height, self.width, 3))
        M = np.max(np.max(self.cMatrix[:,:,-1]))
        
        tipCellLocations = np.zeros((self.height, self.width))
        
        for i in range(0,len(self.tipCells)):
            index1 = self.tipCells[i].y
            index2 = self.tipCells[i].x
            
            tipCellLocations[index1, index2] = 1
        
        vasculaturePicture = self.occupiedCells
        ...
        for i in range(0,self.height):
            for j in range(0,self.width):
                ...
                elif(vasculaturePicture[i,j] == 1):
                    picture[i,j,:] = vasculatureColor
                    
                if(tipCellLocations[i,j] == 0 and vasculaturePicture[i,j] == 0):
                    
                    
                    picture[i,j,:] = minConcentrationColor + (self.cMatrix[i,j,-1]/M)*(maxConcentrationColor - minConcentrationColor)
        return picture
